...
DROP SCHEMA IF EXISTS dashboards.dbsql_warehouse_advisor CASCADE;
CREATE SCHEMA IF NOT EXISTS dashboards.dbsql_warehouse_advisor;
  -- LOCATION 's3://<location>/'; -- Optional location parameter
USE CATALOG dashboards;
USE SCHEMA dbsql_warehouse_advisor;


CREATE OR REPLACE MATERIALIZED VIEW dashboards.dbsql_warehouse_advisor.warehouse_query_history
...
CREATE OR REPLACE MATERIALIZED VIEW dashboards.dbsql_warehouse_advisor.warehouse_usage
SCHEDULE EVERY 1 HOUR
...
CREATE OR REPLACE MATERIALIZED VIEW dashboards.dbsql_warehouse_advisor.warehouse_scaling_events
SCHEDULE EVERY 1 HOUR
...
CREATE OR REPLACE MATERIALIZED VIEW dashboards.dbsql_warehouse_advisor.warehouse_raw_events
SCHEDULE EVERY 1 HOUR
PARTITIONED BY (event_date, workspace_id)
TBLPROPERTIES ("pipelines.autoOptimize.zOrderCols" = "warehouse_id,event_time")
AS 
...
CREATE OR REPLACE VIEW dashboards.dbsql_warehouse_advisor.warehouse_scd
COMMENT 'SQL Warehouse SCD Change History'
AS (
WITH edit_history AS (
    SELECT 
        *
    FROM dashboards.dbsql_warehouse_advisor.warehouse_raw_events
